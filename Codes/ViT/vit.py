...
class ViT(nn.Module):
    def __init__(self,emb_size=16):
        ...
        self.transformer_enc=nn.TransformerEncoder(nn.TransformerEncoderLayer(d_model=emb_size,n_head=2,batch_first=True),num_layers=3)
        self.cls_linear=nn.Linear(in_features=emb_size,out_features=10)

    def forward(self,x):
        x=self.conv(x) #B,1,28,28->B,16,7,7
